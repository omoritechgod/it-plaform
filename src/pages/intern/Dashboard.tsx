...
import { BookOpen, Trophy, DollarSign, Clock, CheckCircle, AlertCircle } from 'lucide-react';
...
export const InternDashboard: React.FC = () => {
  ...
  const achievements = [
    { title: 'First Test Passed', icon: <Trophy className="w-5 h-5" />, date: '2 days ago' },
    { title: 'Module Completed', icon: <CheckCircle className="w-5 h-5" />, date: '1 week ago' },
    { title: 'Project Submitted', icon: <BookOpen className="w-5 h-5" />, date: '2 weeks ago' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <h1 className="text-3xl font-bold text-gray-900">Welcome back, Intern!</h1>
        ...
        <Card className="bg-gradient-to-r from-[#0f266c] to-[#007bff] text-white">
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div className="mb-4 md:mb-0">
              <h2 className="text-2xl font-bold mb-2">Current Stage: {progressData.stage}</h2>
          ...
          <Card>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">₦{progressData.walletBalance.toLocaleString()}</p>
              </div>
                ...
                <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-4">
                    <div className="flex-shrink-0">
                      {task.type === 'learning' && <BookOpen className="w-5 h-5 text-blue-600" />}
                      {task.type === 'project' && <Trophy className="w-5 h-5 text-purple-600" />}
                      {task.type === 'review' && <CheckCircle className="w-5 h-5 text-green-600" />}
                    </div>
                    <div>
            ...
            <div className="space-y-4">
              {achievements.map((achievement, index) => (
                <div key={index} className="flex items-start space-x-3">
                  <div className="text-[#007bff] mt-1">
                    {achievement.icon}
                  </div>
                  <div>
