      ...
      <div>
        <div className="lg:col-span-8 md:overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-lg md:text-2xl font-extrabold text-gray-700">
                Training Modules
              </h2>
              <span className="bg-blue/100 mt-2 text-white px-3 py-1 rounded-full text-sm font-medium">
                  ...
                  <div className="w-full ">
                    ...
                    <div className="pt-4 border-t border-gray-50 flex justify-between items-center">
                      ...
                      <div className="flex items-center text-blue hover:text-blue/50 cursor-pointer font-semibold text-sm">
                        <Link to={`/admin/module/${courseId}/${m.id}`}>
                          Manage Module
                        </Link>
...
const ModuleForm = ({
  ...
  const { mutate: editMutate, isPending: pending } = useMutation({
  ...
  useEffect(() => {
    if (edit) {
      reset({
        ...
        skill_tag: selectedModule?.skill_tag,
        status: selectedModule?.status,
        title: selectedModule?.title,
        resources: {
          pdf: selectedModule?.resources?.pdf,
          video: selectedModule?.resources?.video,
        },
      });
    }
  }, [edit]);
  ...
  return (
    <div className="w-full h-screen fixed bg-black/40 z-40 left-0 top-0">
      <div className="max-w-xl h-[500px] mx-auto my-24 bg-white rounded-2xl shadow-md border overflow-y-auto border-gray-100 p-6 sticky top-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 ">
          <input
            ...
            className="w-20 px-4 py-2 rounded-lg border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"
          ...
          <div>
            ...
            >
              ...
              {edit ? "Update Module" : " Publish Module"}
            </Button>
            <Button
              variant="danger"
              className="w-full flex items-center gap-2 justify-center bg-red-600 hover:bg-red-40 text-white font-semibold py-3 rounded-xl transition-all shadow-md hover:shadow-lg active:scale-95 mt-1"
              onClick={() => {
                setEdit(false);
                setOpenForm(false);
              }}
